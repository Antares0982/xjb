...
struct double_table_t {
    ...
    constexpr double_table_t() {
        ...
        for (int e10 = e10_DN; e10 <= e10_UP + 1; e10++) {
            ...
            // ssrJSON modified block
            max_first_sig_pos = first_sig_pos > max_first_sig_pos
                                    ? first_sig_pos
                                    : max_first_sig_pos;
            // ssrJSON modified block end
...
#define amd64_not_gcc
    ...
    if ((u64)(vi_abs - 2) >= (u64)((2047ull << 52) - 2)) [[unlikely]] {
        ...
        if (vi_abs <= 1) [[likely]] {
            ...
            back = !is_inf && (vi >> 63);
            const char _inf_literal[8] = {'I', 'n', 'f', 'i',
                                          'n', 'i', 't', 'y'};
...
#define use_fast_path_for_integer_xjb 0
    ...
    if (ieee_exponent == 2047) [[unlikely]] {
        return (char*)memcpy(buf - (ieee_significand && (vi >> 63)),
                             ieee_significand ? "NaN\0\0\0\0" : "Infinity", 8) +
               (ieee_significand ? 3 : 8);
    }
    // ssrJSON modified block end
#endif
    ...
    // ssrJSON modified block: max value of `first_sig_pos` is 5; write 5 '0' +
    // 3 '\0' to buffer
...
static inline char* xjb32(float v, char* buf) {
    ...
    if (exp == 255) [[unlikely]] {
        ...
        bool back = sig && (vi >> 31);
        buf -= back;
        return (char*)memcpy(buf, sig ? "NaN\0\0\0\0" : "Infinity", 8) +
               (sig ? 3 : 8);
        // ssrJSON modified block end
    }
    ...
    // ssrJSON modified block: max value of `first_sig_pos` is 4 for xjb32;
    // write 4 '0' to buffer memset(buf, '0', 16);
